...
const verifyAdmin = require('../middleware/vefifyAdmin');
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.status(500).send({ message: 'Failed to fetching the product' });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.status(500).send({ message: 'Failed to dalete the product' });
  }
});
